...
## What `--workers` actually buys you

Parallel derivers don't beat the rate limiter. About 15 of every 16
BIP-39 candidates fail the checksum and skip the API entirely. The 1 in
16 that does hit the API is still capped at `--rate` requests per
...
This is the educational point. You can parallelise the cheap stage to
...
The default `--workers 2` is conservative. It works everywhere from a
Raspberry Pi to a 64-core workstation. To find the actual sweet spot
for your hardware, run the built-in benchmark:
...
the reorder/checker serialisation points eat the gains.
...
Once you find your number, set it once and forget it. The value is
